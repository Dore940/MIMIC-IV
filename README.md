...
**MIMIC-IV data pipeline** 
 The MIMIC-IV data pipeline is a comprehensive system that provides a flexible framework for preparing MIMIC-IV data for subsequent tasks. This pipeline refines the initial data by eliminating anomalies and enabling users to fill in missing data points. It also offers choices for organizing medical attributes using recognized coding systems to reduce complexity.

Users have the ability to personalize their patient groups by adjusting these options, and the customization steps can be saved for ensuring the reproducibility of the entire process. The pipeline generates a well-structured time-series dataset by categorizing the sequential data into uniform time intervals and permitting users to set the length of the time-series based on their preferences. Apart from the data processing components, our pipeline also encompasses two extra sections for modeling and assessment.

In the modeling section, the pipeline incorporates various frequently used successive models for executing predictive tasks. The assessment section provides a range of established techniques for gauging the effectiveness of the generated models. This segment also encompasses alternatives for measuring individual and collective fairness aspects.  
